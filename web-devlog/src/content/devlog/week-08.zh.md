...
summary: >
  ...
  Kevin 后选择用同款树脂填补裂缝，Lucia 同步重新建模做了备用翻模模具，
  最终修补效果出乎意料地好。定制 PCB 到货，Yutong 完成了全部元件及
  WS2812B 灯带的焊接。Lucia 集成了五种状态响应式灯带动画，将构建
  系统迁移至 PlatformIO（ESP-IDF 5.x），并对语音识别进行了调优——
  新增"Release"作为"Spray"的替代触发词。整机组装后五条触发路径
  全部验证通过。
credits:
  - name: Lucia
    initials: L
...
planned_next:
  - id: backup-video
    label: 录制备用演示视频
    description: "重新录制五条触发路径的完整演示，剪辑为正式提交版本，以备现场演示出现网络或环境问题时使用。"
  - id: final-demo
    label: 最终演示
...
- **外壳** — 上下两半全部完工。Yutong 在 Kevin 指导下打印并湿磨好了下底座（60 → 1000 目，水冷打磨）；透明上盖用条锯裁边后钻雾化口，钻穿时树脂从孔缘向外放射状开裂。咨询 Kevin 后选择同款树脂填补，Lucia 同步重新建模备用翻模模具。修补效果惊人地干净，两人共同打磨后裂缝几乎不可见。
- **PCB 与电路** — 定制 PCB 到货。Yutong 完成了所有元件及 WS2812B 灯带的焊接，经多轮调试排除短路后成功上电。
- **固件** — Lucia 集成了五种状态响应式 WS2812B 灯带动画，将构建系统迁移至 PlatformIO（ESP-IDF 5.x）。在真实环境中测试后发现"Spray"识别率较差，新增"Release"作为第二触发词，识别效果显著改善。
...
> **为什么选同款树脂？** 使用相同材料填补，树脂与周围结构形成化学键合而非单纯的表面粘附——接缝从结构上变为一体。打磨后颜色、透明度与表面质感几乎与原件完全一致。与此同时，Lucia 重新建模、制作了一套备用硅胶模具，以备修补失败时重新翻模——最终没用上。
...
固化后，Lucia 与 Yutong 共同打磨。

> 修补效果出乎意料地好——重新打磨后，裂缝几乎不可见，磨砂质感均匀覆盖整个表面。
...
Lucia 集成了 24 颗 WS2812B 灯带（FastLED 3.9.0，GPIO3），实现五种状态响应式动画，为 AuraSync 每个状态提供清晰的视觉语言：
...
整机组装完成后，在真实环境中测试了语音识别，结果好坏参半：

- **"Aura"**（唤醒词）和 **"Stop"**（取消）——识别稳定
- **"Spray"**——在环境噪音中频繁漏识

> **为什么"Spray"容易漏识，"Release"不会？** "Spray"以辅音簇 /spr/ 开头——这是一组高频摩擦音，在 AuraSync 的声学腔体中被大幅衰减。而"Release"以流音辅音开头，接开口元音，在嘈杂环境中鲁棒性强得多。两个词现在映射相同的喷雾动作，相当于免费获得了一个更可靠的触发词。

修复方案：新增 **"Release"** 作为第二触发词，MultiNet7 置信度阈值设为 `0.45f`。当前有效语音指令：`aura` / `spray` / `fragrance` / `release` / `stop`
...
本周在集成测试中拍摄了五条触发路径的原始素材。目前这些视频片段尚未剪辑，也不够正式——计划下周进行正式补录与剪辑，制作一段完整的演示视频。

## 下一步计划

| 完成 | 任务 | 说明 |
|:-:|---|---|
| <input type="checkbox" /> | **录制备用演示视频** | 重新录制五条触发路径并剪辑为正式提交版本，以备现场连接问题时使用。 |
