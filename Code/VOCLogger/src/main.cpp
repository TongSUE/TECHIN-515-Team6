...
void connectWiFi() {
  for (int attempt = 1; attempt <= 3; attempt++) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      delay(2000);  // let network stack stabilise before NTP/DNS
      return;
    }
    Serial.println("\nWiFi timeout, retrying...");
  }
  Serial.println("ERROR: WiFi failed after 3 attempts");
  setError(true);
}
...
void syncNTP() {
  ...
  Serial.println("ERROR: NTP failed after 3 attempts — session ID will be wrong");
}
