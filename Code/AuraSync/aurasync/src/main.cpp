...
#define PIN_LED         21
#define PIN_LED_RING    3       // GPIO3 / D2 → WS2812B DI
#define NUM_LEDS        24
...
static int           iaqClass        = 0;   // 0=Good, 1=Moderate, 2=Poor (updated by IAQ MLP)
...
void setup() {
  ...
  FastLED.addLeds<WS2812B, PIN_LED_RING, GRB>(leds, NUM_LEDS);
...
// ════════════════════════════════════════════════════════════
//  updateLEDRing() — WS2812B 24-LED breathing / animation
// ════════════════════════════════════════════════════════════
void updateLEDRing() {
  static unsigned long lastLedMs = 0;
  if (millis() - lastLedMs < 16) return;  // ~60fps cap
  lastLedMs = millis();

  unsigned long t = millis();

  // ── Helper: sine breathing → brightness 0..maxBri ───────
  auto breathBri = [](unsigned long t_ms, unsigned long period_ms, uint8_t maxBri) -> uint8_t {
    float phase = (float)(t_ms % period_ms) / period_ms;
    float s = (sinf(phase * 2.0f * (float)M_PI - (float)M_PI / 2.0f) + 1.0f) / 2.0f;
    return (uint8_t)(s * maxBri);
  };

  if (sprayState == SPRAY_SPRAYING) {
    // White rotating chase — 3 bright pixels spinning
    int pos = (t / 30) % NUM_LEDS;
    fill_solid(leds, NUM_LEDS, CRGB::Black);
    for (int i = 0; i < 4; i++) {
      uint8_t bri = 255 - i * 55;
      leds[(pos - i + NUM_LEDS) % NUM_LEDS] = CRGB(bri, bri, bri);
    }

  } else if (sprayState == SPRAY_COOLDOWN) {
    // Cyan fade out over cooldown duration
    float progress = (float)(t - sprayStateMs) / (float)SPRAY_CD_MS;
    progress = constrain(progress, 0.0f, 1.0f);
    uint8_t bri = (uint8_t)((1.0f - progress) * 120);
    fill_solid(leds, NUM_LEDS, CRGB(0, bri, bri));

  } else if (voiceWindow) {
    // Purple quick double-pulse then steady
    unsigned long elapsed = t - voiceWindowStart;
    uint8_t bri = (elapsed < 200 || (elapsed > 300 && elapsed < 500)) ? 200 : 80;
    fill_solid(leds, NUM_LEDS, CRGB(bri / 2, 0, bri));

  } else if (sysMode == MODE_SLEEP) {
    // Very dim slow blue breathing (8s cycle)
    uint8_t bri = breathBri(t, 8000, 18);
    fill_solid(leds, NUM_LEDS, CRGB(0, 0, bri));

  } else {
    // AWAKE — color and speed by IAQ class
    switch (iaqClass) {
      case 0: {  // Good — green slow breathing 4s
        uint8_t bri = breathBri(t, 4000, 80);
        fill_solid(leds, NUM_LEDS, CRGB(0, bri, 0));
        break;
      }
      case 1: {  // Moderate — yellow medium 2s
        uint8_t bri = breathBri(t, 2000, 120);
        fill_solid(leds, NUM_LEDS, CRGB(bri, bri / 2, 0));
        break;
      }
      case 2: {  // Poor — orange fast pulse 0.5s
        uint8_t bri = breathBri(t, 500, 200);
...
bool initESPSR() {
  ...
  esp_mn_commands_add(CMD_ID_SPRAY, "fragrance");
  esp_mn_commands_add(CMD_ID_SPRAY, "release");
  esp_mn_commands_add(CMD_ID_STOP,  "stop");
  esp_mn_commands_update();
  multinet->set_det_threshold(model_data, 0.45f);
  return true;
}
...
void srProcessingTask(void *pvParam) {
  ...
  while (true) {
    ...
    // Debug: peak amplitude every ~100 chunks
    static int dbgCnt = 0;
    if (++dbgCnt % 100 == 0) {
      int32_t peakL = 0, peakR = 0;
      for (int i = 0; i < samplesRead; i++) {
        int32_t l = abs(i32Buf[i * 2] >> 16);
        int32_t r = abs(i32Buf[i * 2 + 1] >> 16);
        if (l > peakL) peakL = l;
        if (r > peakR) peakR = r;
      }
      Serial.printf("[MIC] L=%d R=%d samples=%d\n", (int)peakL, (int)peakR, samplesRead);
    }
